...
<?
$DbLink = new DB;
$DbLink->query("SELECT uuid,regionName,serverIP,locX,locY,serverHttpPort,owner_uuid FROM ".C_REGIONS_TBL." Order by locX");
while(list($UUID,$regionName,$serverIP,$locX,$locY,$serverHttpPort,$owner) = $DbLink->next_record()){

$DbLink1 = new DB;
$DbLink1->query("SELECT username,lastname FROM ".C_USERS_TBL." where uuid='$owner'");
list($firstN,$lastN) = $DbLink1->next_record();

$SERVERCHECK="http://$serverIP:$serverHttpPort/simstatus/";
...
?>
    ...
    var tmp_region_image = new Img("http://<?=$serverIP?>:<?=$serverHttpPort?>/index.php?method=regionImage<?=$UUID?>",<?=$size?>,<?=$size?>);
